...
class BrandSyncProcessor extends EntitySyncProcessor {
  BrandSyncProcessor(this._remote, {AuditService? auditService}) : _audit = auditService;

  final BrandRemoteDataSource _remote;
  final AuditService? _audit;

  @override
  String get entityType => Brand.entityTypeName;
  ...
  Future<SyncProcessResult> push(Map<String, dynamic> queueItem) async {
    try {
      ...
      await _audit?.log(
        ...
        tenantId: tenantId,
        entityId: entityId,
        metadata: {'change_type': 'brand_sync', 'operation': operation.value},
      );
