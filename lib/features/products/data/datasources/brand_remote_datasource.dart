...
class BrandRemoteDataSource {
  BrandRemoteDataSource({SupabaseClient? client}) : _client = client ?? Supabase.instance.client;
  ...
  Future<void> push({
    ...
    final table = _client.from('brands');
    switch (operation) {
    ...
    AppLogger.debug('Brand remote push: ${operation.value} ${payload['id']}');
  }
