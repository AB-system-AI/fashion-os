...
class _TransfersPageState extends ConsumerState<TransfersPage> {
  ...
  Future<void> _load() async {
    ...
    final page = await ref.read(transferServiceProvider).list(user!.tenantId!);
    if (!mounted) return;
    setState(() {
      _items = page.items;
      _loading = false;
  ...
  Future<void> _createSample() async {
    final user = ref.read(authControllerProvider).user;
    if (user == null) return;
    await ref.read(transferServiceProvider).create(
          user: user,
          input: const TransferInput(fromAccountId: 'cash-1', toAccountId: 'bank-1', amount: 100),
          fromBalance: 500,
        );
    await _load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canManage = ref.watch(permissionCheckProvider(TransferPermissions.manage));
    return AppScaffold(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text('${t.status.value} · ${t.amount.toStringAsFixed(2)}'),
                    ),
                  );
                },
              ),
