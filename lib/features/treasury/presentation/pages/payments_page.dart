...
class _PaymentsPageState extends ConsumerState<PaymentsPage> {
  List<PaymentVoucher> _items = const [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final page = await ref.read(paymentServiceProvider).list(user!.tenantId!);
    if (!mounted) return;
    setState(() {
      _items = page.items;
      _loading = false;
  ...
  Future<void> _createSample() async {
    ...
    await ref.read(paymentServiceProvider).create(
          user: user,
          input: const PaymentInput(payeeName: 'Supplier A', amount: 250, accountId: 'bank-1'),
        );
    await _load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canManage = ref.watch(permissionCheckProvider(PaymentPermissions.manage));
    return AppScaffold(
      appBar: const AppAppBar(title: Text('Payments')),
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text('${v.payeeName} · ${v.amount.toStringAsFixed(2)}'),
                    ),
                  );
                },
              ),
