...
class ErrorLogsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppScaffold(
      appBar: const AppAppBar(title: Text('Error Logs')),
      body: FutureBuilder(
        future: ref.read(errorLogServiceProvider).listUnresolved(user),
        builder: (context, snapshot) {
          if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
          final result = snapshot.data!;
          if (result.isFailure) return Center(child: Text(result.failureOrNull?.message ?? 'Error'));
          final errors = result.dataOrNull ?? [];
          if (errors.isEmpty) return const Center(child: Text('No unresolved errors'));
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return ListTile(
                ...
                subtitle: Text('${e.severity.value} · ${e.occurredAt.toLocal()}'),
