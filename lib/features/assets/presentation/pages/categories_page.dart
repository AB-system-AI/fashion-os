...
class _CategoriesPageState extends ConsumerState<CategoriesPage> {
  List<AssetCategory> _items = const [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final items = await ref.read(assetServiceProvider).listCategories(user!.tenantId!);
    if (!mounted) return;
    setState(() {
      _items = items;
      _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      appBar: const AppAppBar(title: Text('Asset Categories')),
      body: _loading
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      title: Text(c.name),
                      subtitle: Text(c.code ?? 'No code'),
                      trailing: Text('${c.defaultUsefulLifeMonths} mo'),
                    ),
                  );
                },
              ),
