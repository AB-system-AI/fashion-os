...
class _ShipmentListPageState extends ConsumerState<ShipmentListPage> {
  ...
  Future<void> _load() async {
    ...
    final items = await ref.read(shipmentServiceProvider).listByStatus(user!.tenantId!, ShipmentStatus.pending);
    if (!mounted) return;
    setState(() {
      _items = items;
      _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      appBar: const AppAppBar(title: Text('Shipments')),
      body: _loading
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      onTap: () => context.push(SalesRoutePaths.shipmentDetail(s.id)),
                    ),
                  );
                },
              ),
